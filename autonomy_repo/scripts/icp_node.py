...
class ICPNode(Node):
    ...
    def pcd_callback(self, msg: PointCloud2):
        ...
        pts = (self.lidar_pose[:3,:3]@pts.T + self.lidar_pose[:3,-1:]).T
        #self.get_logger().info(str(type(pts)))
        curr_pcd = o3d.geometry.PointCloud()
        curr_pcd.points = o3d.utility.Vector3dVector(pts)
        curr_pcd = curr_pcd.uniform_down_sample(10)

        ### Task 3.5 ###

        ### TODO: Task 3.6 ###
        if self.transformation is None:
            self.transformation = open3d_icp(curr_pcd, self.prev_pcd, np.eye(4))
        else:
            self.transformation = open3d_icp(curr_pcd, self.prev_pcd, self.transformation)
        ### Task 3.6 ###
