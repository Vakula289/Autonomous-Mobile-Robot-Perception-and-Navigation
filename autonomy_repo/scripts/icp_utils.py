...
def nearest_neighbor(src: np.ndarray, dst: np.ndarray, radius=0.01):
    ...
    nn.fit(dst)
    distances, indices = nn.kneighbors(src, 1, return_distance=True)
    ########### YOUR CODE ENDS HERE ###########
...
def icp(
    ...
    for i in range(max_iterations):
        distances, indices_nn = nearest_neighbor(src[:m,:].T, B)
        T,R, t = best_fit_transform(src[:m,:].T, B[indices_nn])

        src[:m, :] = np.dot(R, src[:m,:]) + t[:, np.newaxis]

        avg_error = np.mean(distances)
        if abs(avg_error- prev_error)<tolerance:
            break
        else:
